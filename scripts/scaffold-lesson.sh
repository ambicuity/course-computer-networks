...
Examples:
  scripts/scaffold-lesson.sh 05-network-layer 07-ip-addressing
  scripts/scaffold-lesson.sh 05-network-layer 07-ip-addressing "IP Addressing"
...
if [[ -z "$REPO_ROOT" ]]; then
  echo "error: run this from inside the course-computer-networks repo" >&2
  exit 1
fi
...
if [[ ! "$LESSON" =~ ^[0-9]{2}-[a-z0-9-]+$ ]]; then
  echo "error: lesson slug must match NN-kebab-case (e.g. 07-ip-addressing)" >&2
  exit 1
fi
