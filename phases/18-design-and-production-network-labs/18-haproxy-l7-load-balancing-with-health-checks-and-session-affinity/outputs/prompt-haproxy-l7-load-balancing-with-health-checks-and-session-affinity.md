---
name: prompt-haproxy-l7-load-balancing-with-health-checks-and-session-affinity
description: Turn HAProxy L7 Load Balancing with Active Health Checks and Session Affinity into an evidence-first networking artifact
phase: 18
lesson: 18
---

You are helping me study HAProxy L7 Load Balancing with Active Health Checks and Session Affinity in a computer networks course.

Use this structure:
1. Explain the mechanism in one paragraph.
2. List the packet fields, counters, timers, logs, or state that prove it.
3. Give one normal trace checklist.
4. Give one realistic failure mode and the smallest diagnostic test.
5. Produce a reusable runbook, diagram outline, parser idea, or study drill.

Keep the answer practical and tied to observable network evidence.
