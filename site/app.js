(function () {
  ...
  function computeStats() {
    ...
    var hasProgress = !!window.CNProgress;
    for (var i = 0; i < PHASES.length; i++) {
      ...
      for (var j = 0; j < lessons.length; j++) {
        ...
        if (hasProgress && lessons[j].url) {
          var lp = window.CNProgress.extractPath(lessons[j].url);
          if (lp) userDone = window.CNProgress.isLessonComplete(lp);
        }
    ...
    for (var i = 0; i < PHASES.length; i++) {
      ...
      for (var j = 0; j < p.lessons.length; j++) {
        ...
        if (hasProgress && p.lessons[j].url) {
          var lp = window.CNProgress.extractPath(p.lessons[j].url);
          if (lp) userDone = window.CNProgress.isLessonComplete(lp);
        }
  ...
  function renderModalLessons(p) {
    ...
    for (var i = 0; i < p.lessons.length; i++) {
      ...
      var userComplete = hasProgress && lessonPath && window.CNProgress.isLessonComplete(lessonPath);
    ...
    for (var t = 0; t < toggles.length; t++) {
      toggles[t].addEventListener('click', function (e) {
        ...
        if (!path || !window.CNProgress) return;
        if (window.CNProgress.isLessonComplete(path)) {
          window.CNProgress.unmarkLessonComplete(path);
        } else {
          window.CNProgress.markLessonComplete(path);
        }
      });
    }
  ...
  if (window.CNProgress) {
    window.CNProgress.onChange(function () {
