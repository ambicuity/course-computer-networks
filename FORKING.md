...
- Keep `ROADMAP.md` as the progress source of truth (the website status badges are parsed from it).
...
- Add lab setup instructions for school machines or network lab environments (Wireshark, tcpdump, packet-capture permissions).
- Add assignments that wrap existing lessons instead of rewriting the lesson body.

Avoid editing generated site output directly. Update the Markdown sources first, then rebuild with:
...
3. Resolve conflicts in source files (`docs/en.md`, `README.md`, `ROADMAP.md`), not generated output (`site/data.js`).
